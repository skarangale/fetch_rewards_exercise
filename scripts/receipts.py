...
INSERT_RECEIPT_RECORD_SQL_PATH = os.path.join(INSERT_QUERY_FOLDER, "insert_receipt_record.sql")
INSERT_RECEIPT_ITEM_RECORD_SQL_PATH = os.path.join(INSERT_QUERY_FOLDER, "insert_receipt_item_record.sql")
...
def load_receipts_in_db():
    # Truncate tables before loading
    execute_query('DELETE FROM receipt_item;')
    execute_query('DELETE FROM receipt;')
    
    user_ids = set(users.get_user_ids())

    receipt_records: dict[str, tuple] = {}
    ...
    with open(RECEIPTS_JSON_PATH, "r") as file:
        for line in file:
            ...
            receipt_records[id] = (receipt_record)
            ...
            if "rewardsReceiptItemList" in receipt:
                for item in receipt["rewardsReceiptItemList"]:
                    ...
                    if receipt_item_record:
                        receipt_items_records.append(receipt_item_record)
    
    with open(INSERT_RECEIPT_RECORD_SQL_PATH, "r") as file:
        query = file.read()
        executemany(query, receipt_records.values())
    
    with open(INSERT_RECEIPT_ITEM_RECORD_SQL_PATH, "r") as file:
        query = file.read()
        executemany(query, receipt_items_records)
...
def parse_receipt_item(item, record_id):
    ...
    target_price = item["targetPrice"] if "targetPrice" in item else None
    ...
    return (barcode, brand_code, competitive_product, competitor_rewards_group, deleted, description, discounted_item_price, final_price,
            ...
            price_after_coupon, quantity_purchased, receipt_id, rewards_group, rewards_product_partner_id, target_price, 
